...
from config import ENABLE_PLATEGA, REFERRAL_PERCENT
...
async def _credit_payment(transaction_id: str, user_id: int, amount: float):
    """Зачисляет оплату на баланс и начисляет реферальный бонус пригласившему."""
    ...
    if referrer_id:
        ...
        await db.add_log(referrer_id, "referral_bonus", details=f"from={user_id} bonus={bonus}")
